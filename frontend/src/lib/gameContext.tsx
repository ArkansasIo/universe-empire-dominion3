...
export function GameProvider({ children }: { children: React.ReactNode }) {
      ...
      setQueue(prev => {
        ...
        finished.forEach(item => {
           if (item.type === "building") {
             if (Object.prototype.hasOwnProperty.call(buildings, item.id)) {
               setBuildings(b => ({ ...b, [item.id]: b[item.id as keyof Buildings] + 1 }));
             } else {
               setOrbitalBuildings(ob => ({ ...ob, [item.id]: (ob[item.id] || 0) + 1 }));
             }
  ...
  const updateBuilding = async (building: keyof Buildings | string, name: string, time: number = 5000) => {
    ...
    const isPlanetBuilding = Object.prototype.hasOwnProperty.call(buildings, building);

    // Orbital/moon/station building path (frontend-only queue + local state completion in main loop)
    if (!isPlanetBuilding) {
      const orbitalLevel = orbitalBuildings[building] || 0;
      const baseMetal = 200;
      const baseCrystal = 100;
      const baseDeut = 0;

      const metalCost = Math.floor(baseMetal * Math.pow(2, orbitalLevel));
      const crystalCost = Math.floor(baseCrystal * Math.pow(2, orbitalLevel));
      const deuteriumCost = Math.floor(baseDeut * Math.pow(2, orbitalLevel));
      ...
      if (
        ...
        addEvent("Build Failed", "Insufficient resources for orbital construction", "danger");
        return;
      }

      setResources(prev => ({
        ...prev,
        metal: prev.metal - metalCost,
        crystal: prev.crystal - crystalCost,
        deuterium: prev.deuterium - deuteriumCost
      }));

      const adjustedTime = (time * Math.pow(1.15, orbitalLevel)) / (config?.gameSpeed || 1);
      const now = Date.now();
      setQueue(prev => [...prev, {
        id: building,
        name: name,
      ...
      addEvent("Build Started", `${name} construction started`, "success");
      return;
    }
    ...
    try {
      ...
      const response = await apiRequest("POST", "/api/game/build", {
        building: building,
        level: (buildings[building as keyof Buildings] || 0) + 1
      });
      ...
      if (response.success) {
        ...
        const adjustedTime = (time * Math.pow(1.15, buildings[building as keyof Buildings] || 0)) / (config?.gameSpeed || 1);
        const now = Date.now();
        setQueue(prev => [...prev, {
          id: building,
          name: name,
