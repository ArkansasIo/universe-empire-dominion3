...
import { Mail, Send, Trash2, Reply, AlertCircle, User, Shield, Flag } from "lucide-react";
                       ...
                       {inbox.map(msg => (
                          ...
                          >
                             <div className="flex justify-between items-start mb-1">
                                <span className={cn("text-sm", !msg.read ? "text-slate-900" : "text-slate-600")}>{msg.from}</span>
              ...
              {activeMessage ? (
                 <>
                    ...
                    <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-2 rounded-b-lg">
                       {activeMessage.from !== "Commander" && (
                          <Button variant="outline" onClick={() => {
                             setComposeTo(activeMessage.from);
                             setComposeSubject(`Re: ${activeMessage.subject}`);
                             // Need to switch tab to compose, simpler to just pre-fill
                          }}>
